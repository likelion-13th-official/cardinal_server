...
public class GoodsService {
    ...
    public PageDto<GoodsResponse> searchGoods(String query, int page, Long userId) {
        ...
        SearchHits<GoodsDocument> searchHits = elasticsearchOperations.search(nativeQuery, GoodsDocument.class, IndexCoordinates.of("goods"));
        ...
        // 최종응답
        List<GoodsResponse> goodsResponses = goodsIds.stream()
                .map(goodsMap::get)
                .map(goods -> GoodsResponse.from(goods, scrappedGoodsIds.contains(goods.getId())))
                .collect(Collectors.toList());
